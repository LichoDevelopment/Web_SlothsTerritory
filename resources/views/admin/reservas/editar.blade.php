            ...
            <form action="{{ route('reserva.actualizar', ['id' => $reserva->id]) }}" method="post">
                ...
                <section class="row mb-3">
                    <article class="col-6">
                        ...
                        <input class="form-control" name="nombre_cliente" value="{{ $reserva->nombre_cliente }}" required>
                    </article>
                    <article class="col-6">
                        <label for="cantidad_adultos">Adultos</label>
                        <input type="number" min="0" class="form-control" id="cantidad_adultos"
                            name="cantidad_adultos" value="{{ $reserva->cantidad_adultos }}" required>
                    </article>
                </section>
                <section class="row mb-3">
                    <article class="col-6">
                    ...
                    <article class="col-6">
                        ...
                        <input type="number" min="0" class="form-control" id="cantidad_niños_gratis"
                            name="cantidad_niños_gratis" required value="{{ $reserva->cantidad_niños_gratis }}">
                    </article>
                </section>
                <section class="row mb-3">
                    <article class="col-6">
                        ...
                        <select class="custom-select" id="tour" name="id_tour" data-precios="{{ $precios }}"
                            required>
                            <option selected value="{{ $reserva->id_tour }}">
                                {{ $reserva->tour->nombre }}
                            </option>
                ...
                <section class="row mb-3">
                    ...
                    <article class="col-6">
                        ...
                        <select class="custom-select" name="id_horario" id="horarios" data-horarios="{{ $horarios }}"
                            required>
                ...
                <section class="row mb-3">
                    ...
                    <article class="col-6">
                        ...
                        <input type="text" class="form-control" id="precio_adulto" value="" disabled>
                    </article>
                    <article class="col-6">
                        <label for="precio_nino">Precio niños</label>
                        <input type="text" class="form-control" id="precio_nino" value="" disabled>
                    </article>
                </section>
                <section class="row mb-3">
                    <article class="col-6">
                        <label for="comision_agencia">Comision</label>
                        <input type="number" step=".01" min="0" name="comision_agencia" id="comision"
                            class="form-control" required value="{{ $reserva->comision_agencia }}">
                    </article>
                    <article class="col-6">
                        <label for="descuento">Descuento</label>
                        <input type="number" step=".01" min="0" value="0" name="descuento"
                            id="descuento" class="form-control" required value="{{ $reserva->descuento }}">
                    </article>
                </section>
                <section class="row mb-3">
                    <article class="col-6">
                        <label for="monto_total">Precio</label>
                        <input type="number" min="0" step=".01" name="monto_total" id="monto_total"
                            class="form-control" required value="{{ $reserva->monto_total }}">
                    </article>
                    <article class="col-6">
                        <label for="monto_con_descuento">Precio con descuento</label>
                        <input type="number" min="0" step=".01" name="monto_con_descuento"
                ...
                <section class="row mb-3">
                    <article class="col-6">
                        ...
                        <input type="number" min="0" step=".01" name="monto_neto" id="monto_neto"
                            class="form-control" required value="{{ $reserva->monto_neto }}">
                    </article>
                    <article class="col-6">
                        <label for="factura">Factura</label>
                        <input type="text" name="factura" class="form-control" value="{{ $reserva->factura }}">
                    </article>
                </section>
                <!-- Ejemplo de sección para métodos de pago -->
                <section class="row mb-3">
                    <article class="col-4">
                        <label for="efectivo_crc">Efectivo CRC</label>
                        <input type="number" step="0.01" name="efectivo_crc" id="efectivo_crc" class="form-control"
                            value={{ $reserva->movimientoCaja->efectivo_crc ?? 0 }}>
                    </article>
                    <article class="col-4">
                        <label for="efectivo_usd">Efectivo USD</label>
                        <input type="number" step="0.01" name="efectivo_usd" id="efectivo_usd" class="form-control"
                            value={{ $reserva->movimientoCaja->efectivo_usd ?? 0 }}>
                    </article>
                    <article class="col-4">
                        <label for="tarjeta_crc">Tarjeta CRC</label>
                        <input type="number" step="0.01" name="tarjeta_crc" id="tarjeta_crc" class="form-control"
                            value={{ $reserva->movimientoCaja->tarjeta_crc ?? 0 }}>
                    </article>
                </section>

                <section class="row mb-3">
                    <article class="col-4">
                        <label for="tarjeta_usd">Tarjeta USD</label>
                        <input type="number" step="0.01" name="tarjeta_usd" id="tarjeta_usd" class="form-control"
                            value={{ $reserva->movimientoCaja->tarjeta_usd ?? 0 }}>
                    </article>
                    <article class="col-4">
                        <label for="transferencia_crc">Transferencia CRC</label>
                        <input type="number" step="0.01" name="transferencia_crc" id="transferencia_crc"
                            class="form-control" value={{ $reserva->movimientoCaja->transferencia_crc ?? 0 }}>
                    </article>
                    <article class="col-4">
                        <label for="transferencia_usd">Transferencia USD</label>
                        <input type="number" step="0.01" name="transferencia_usd" id="transferencia_usd"
                            class="form-control" value={{ $reserva->movimientoCaja->transferencia_usd ?? 0 }}>
                    </article>
                </section>
                <section class="row mb-3">
                    <article class="col-6">
                        ...
                        <div class="form-check">
                            <input type="checkbox" class="form-check-input" id="pendiente_cobrar"
                                name="pendiente_cobrar" value="1" {{ $reserva->pendiente_cobrar ? 'checked' : '' }}>
                            <label class="form-check-label" for="pendiente_cobrar">Marcar si está pendiente de
                                pago</label>
                        </div>
                    </article>
                </section>
...
@section('scripts')
    <script>
        ...
        precio_adulto.addEventListener('change', event => {
            actualizarPrecio({
                "cantidad_adultos": cantidad_adultos.value,
        ...
        agencia.addEventListener('change', async event => {
            ...
            comision.value = (Number(cantidad_adultos.value) + Number(cantidad_ninos.value)) * Number(
